...
class CachedDataset:
    ...
    def __init__(self, dataset, normal_class, data_root, evidence_dir):
        ...
        for split in ("train", "test"):
            ...
            parity[split] = {
                "count": len(source),
                "transform_max_abs_error": max(errors),
            }
        write_json(
            Path(evidence_dir) / f"{dataset}_class{normal_class}_data.json",
            {
                **parity,
                "cache_sha256": sha256(cache),
                "minmax_source": "pinned_author_constants",
                "test_fitted": False,
                "train_ids": value["train"][2].tolist(),
                "test_ids": value["test"][2].tolist(),
            },
        )

    def loaders(
        self, batch_size, shuffle_train=True, shuffle_test=False, num_workers=0
    ):
        return (
            DataLoader(
                self.train_set,
            ...
            DataLoader(
                self.test_set, batch_size, shuffle=shuffle_test, num_workers=num_workers
            ),
        )
...
def objective_loss(outputs, inputs, phase, objective, center, radius, nu):
    if phase == "ae":
        return torch.mean(
            torch.sum((outputs - inputs) ** 2, dim=tuple(range(1, outputs.dim())))
        ), None
...
def feature_diagnostics(net, data):
    ...
    with torch.no_grad():
        ...
        value = {
            "sample_count": len(x),
            "mean_feature_variance": float(z.var(0, unbiased=False).mean()),
...
def train_phase(
    ...
    phase_config = (
        ...
        if phase == "ae"
            ...
            "milestone": config["svdd_milestone"],
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, [phase_config["milestone"]], gamma=config["lr_gamma"]
    )
    ...
    if checkpoint.exists():
        ...
        assert (
            previous["target_epochs"] == epochs and previous["objective"] == objective
        )
        ...
        trainer = DeepSVDDTrainer(objective, 0.0, None, config["nu"], device="cuda")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        loss_sum, gradient_sum, batches, seen = 0.0, 0.0, 0, 0
        for inputs, _, _ in train_loader:
            inputs = inputs.cuda()
            optimizer.zero_grad()
            outputs = net(inputs)
            loss, distances = objective_loss(
                outputs, inputs, phase, objective, center, radius, config["nu"]
            )
            ...
            if (
                phase == "svdd"
                and objective == "soft-boundary"
                and epoch >= config["warmup_epochs"]
            ):
                radius.data = torch.tensor(
                    get_radius(distances, config["nu"]), device="cuda"
                )
            loss_sum += loss.item()
        ...
        print(
            json.dumps({"run": str(out.relative_to(ROOT)), "phase": phase, **row}),
            flush=True,
        )
...
def predict(net, data, center, radius, objective):
    ...
    with torch.no_grad():
        for inputs, targets, indices in loader:
            ...
            if objective == "soft-boundary":
                distances -= radius**2
            for i, y, score in zip(
                indices.tolist(), targets.tolist(), distances.cpu().tolist()
            ):
                rows.append(
                    {
                        "sample_id": i,
                        "original_label": data.original_labels[i],
                        "anomaly_label": y,
                        "score": score,
                    }
                )
...
def export_result(net, data, center, radius, objective, out, metadata):
    ...
    error = float(
        np.max(
            np.abs(
                np.array([r["score"] for r in rows])
                - np.array([r["score"] for r in replay_rows])
            )
        )
    )
    assert error <= 1e-6
    ...
    result = {
        **metadata,
        ...
        "prediction_sha256": sha256(out / "predictions.csv"),
        "checkpoint_replay_max_abs_error": error,
        "feature_after": feature_diagnostics(net, data),
        "complete": True,
    }
...
def main():
    ...
    if args.output_root is None:
        args.output_root = ROOT / "results/native/deep_svdd" / config["target"]
    config["ae_settings"] = config["ae"][args.dataset]
    config["dataset"], config["normal_class"], config["seed"] = (
        args.dataset,
        args.normal_class,
        args.seed,
    )
    ...
    data = CachedDataset(
        ...
        args.normal_class,
        args.data_root,
        args.output_root / "data_evidence",
    )
    ...
    ae_saved = torch.load(
        output / "ae/latest.pt", weights_only=False, map_location="cuda"
    )
    ...
    for objective in config["objectives"]:
        ...
        net, center, radius, history = train_phase(
            net,
            data,
            config,
            "svdd",
        ...
        result = export_result(
            ...
            out,
            {
                ...
                "ae_epochs": len(ae_history),
                "svdd_epochs": len(history),
                "nu": config["nu"],
