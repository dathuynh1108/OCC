...
class FixtureDataset:
    ...
    def __init__(self, shape):
        ...
        self.train_set = self.test_set = TensorDataset(
            inputs, torch.zeros(6), torch.arange(6)
        )
...
def state_errors(a, b):
    ...
    for key in a.state_dict():
        ...
        values.append(
            {"key": key, "max": float(error.max()), "mean": float(error.mean())}
        )
...
def main(argv=None):
    ...
    for dataset, shape in [("mnist", (1, 28, 28)), ("cifar10", (3, 32, 32))]:
        ...
        for phase, objective in [
            ...
            initial = (build_autoencoder if phase == "ae" else build_network)(
                dataset + "_LeNet"
            ).cuda()
            ...
            if phase == "ae":
                trainer = AETrainer(
                    ...
                    lr_milestones=(1,),
                    batch_size=4,
                    weight_decay=config["ae_settings"]["weight_decay"],
                    device="cuda",
                )
            else:
            ...
            restore_rng(state)
            candidate, center, radius, _ = train_phase(
                candidate, data, config, phase, objective, path, epochs_override=1
            )
            ...
            restore_rng(state)
            full, _, _, _ = train_phase(
                full, data, config, phase, objective, path / "full", epochs_override=2
            )
            restore_rng(state)
            split, _, _, _ = train_phase(
                split,
                data,
                config,
                phase,
            ...
            output.append(
                {
                    ...
                    "phase": phase,
                    "objective": objective,
                    "parameter_buffer_errors": errors,
                    "gradient_max_abs_error": max(gradients),
                    "resume_max_abs_error": max(r["max"] for r in resume),
                    "tolerance": 1e-7,
                    "upstream_same_runtime_parity": True,
