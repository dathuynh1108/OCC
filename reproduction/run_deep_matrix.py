...
from .common import ROOT, load_plan, sha256, write_json
...
write_json(root / 'launch.json', {'run_plan_sha256': sha256(ROOT / 'run_plan.json'),
                                 'parity_sha256': sha256(parity_file), 'started_unix': time.time(),
                                 'full_matrix_objective_rows': 400})
failures = []
for dataset in plan['datasets']:
    for objective in plan['objectives']:
        result = ROOT / f'artifacts/reproduction/smoke/deep_svdd/{dataset}/class_0/seed_1/{objective}/result.json'
    ...
    for normal in plan['classes']:
        for seed in plan['seeds']:
            command = [sys.executable, '-m', 'reproduction.deep_svdd', '--dataset', dataset,
                       '--normal-class', str(normal), '--seed', str(seed)]
            print('RUN', ' '.join(command), flush=True)
            result = subprocess.run(command, cwd=ROOT)
            if result.returncode:
                failures.append({'dataset': dataset, 'normal_class': normal, 'seed': seed,
                                 'returncode': result.returncode, 'command': command})
                write_json(root / 'failures.json', failures)
write_json(root / 'matrix_exit.json', {'failure_count': len(failures), 'finished_unix': time.time()})
